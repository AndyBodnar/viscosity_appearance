        ...
        if currentZone then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                elseif currentZone.name == "clothing" then
                    TriggerEvent("viscosity_appearance:client:openClothingShopMenu")
                elseif currentZone.name == "barber" then
                    OpenBarberShop()
                elseif currentZone.name == "tattoo" then
                    OpenTattooShop()
...
RegisterNetEvent("viscosity_appearance:client:OpenClothingRoom", function()
    local clothingRoom = Config.ClothingRooms[currentZone.index]
    local outfits = GetPlayerJobOutfits(clothingRoom.job)
    TriggerEvent("viscosity_appearance:client:openJobOutfitsMenu", outfits)
end)

RegisterNetEvent("viscosity_appearance:client:OpenPlayerOutfitRoom", function()
    local outfitRoom = Config.PlayerOutfitRooms[currentZone.index]
    OpenOutfitRoom(outfitRoom)
end)
