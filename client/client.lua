...
local function LoadPlayerUniform(reset)
    if reset then
        TriggerServerEvent("viscosity_appearance:server:syncUniform", nil)
        return
    end
    lib.callback("viscosity_appearance:server:getUniform", false, function(uniformData)
        if not uniformData then
            return
        end
        if Config.BossManagedOutfits then
            local result = lib.callback.await("viscosity_appearance:server:getManagementOutfits", false, uniformData.type, Framework.GetGender())
            local uniform = nil
            for i = 1, #result, 1 do
            ...
            if not uniform then
                TriggerServerEvent("viscosity_appearance:server:syncUniform", nil) -- Uniform doesn't exist anymore
                return
            end

            TriggerEvent("viscosity_appearance:client:changeOutfit", uniform)
        else
            ...
            TriggerEvent("viscosity_appearance:client:loadJobOutfit", uniform)
        end
    end)
end
...
function InitAppearance()
    ...
    lib.callback("viscosity_appearance:server:getAppearance", false, function(appearance)
...
function InitializeCharacter(gender, onSubmit, onCancel)
    ...
    TriggerServerEvent("viscosity_appearance:server:ChangeRoutingBucket")
    client.startPlayerCustomization(function(appearance)
        if (appearance) then
            TriggerServerEvent("viscosity_appearance:server:saveAppearance", appearance)
            if onSubmit then
                onSubmit()
            end
        ...
        Framework.CachePed()
        TriggerServerEvent("viscosity_appearance:server:ResetRoutingBucket")
    end, config)
end

function OpenShop(config, isPedMenu, shopType)
    lib.callback("viscosity_appearance:server:hasMoney", false, function(hasMoney, money)
        ...
        client.startPlayerCustomization(function(appearance)
            if appearance then
                if not isPedMenu then
                    TriggerServerEvent("viscosity_appearance:server:chargeCustomer", shopType)
                end
                TriggerServerEvent("viscosity_appearance:server:saveAppearance", appearance)
            else
                lib.notify({
...
RegisterNetEvent("viscosity_appearance:client:openClothingShop", OpenClothingShop)

RegisterNetEvent("viscosity_appearance:client:importOutfitCode", function()
    ...
    if outfitCode ~= nil then
        Wait(500)
        lib.callback("viscosity_appearance:server:importOutfitCode", false, function(success)
            if success then
                lib.notify({
...
RegisterNetEvent("viscosity_appearance:client:generateOutfitCode", function(id)
    lib.callback("viscosity_appearance:server:generateOutfitCode", false, function(code)
        if not code then
            lib.notify({
...
RegisterNetEvent("viscosity_appearance:client:saveOutfit", function()
    ...
    if outfitName then
        Wait(500)
        lib.callback("viscosity_appearance:server:getOutfits", false, function(outfits)
            ...
            TriggerServerEvent("viscosity_appearance:server:saveOutfit", outfitName, pedModel, pedComponents, pedProps)
        end)
    end
end)

RegisterNetEvent('viscosity_appearance:client:updateOutfit', function(outfitID)
    ...
    lib.callback("viscosity_appearance:server:getOutfits", false, function(outfits)
        ...
        TriggerServerEvent("viscosity_appearance:server:updateOutfit", outfitID, pedModel, pedComponents, pedProps)
    end)
end)
...
local function RegisterChangeOutfitMenu(id, parent, outfits, mType)
    ...
    for i = 1, #outfits, 1 do
        changeOutfitMenu.options[#changeOutfitMenu.options + 1] = {
            ...
            event = "viscosity_appearance:client:changeOutfit",
            args = {
                type = mType,
                name = outfits[i].name,
...
local function RegisterUpdateOutfitMenu(id, parent, outfits)
    ...
    for i = 1, #outfits, 1 do
        updateOutfitMenu.options[#updateOutfitMenu.options + 1] = {
            title = outfits[i].name,
            description = outfits[i].model,
            event = "viscosity_appearance:client:updateOutfit",
            args = outfits[i].id
        }
    end
...
local function RegisterGenerateOutfitCodeMenu(id, parent, outfits)
    ...
    for i = 1, #outfits, 1 do
        generateOutfitCodeMenu.options[#generateOutfitCodeMenu.options + 1] = {
            title = outfits[i].name,
            description = outfits[i].model,
            event = "viscosity_appearance:client:generateOutfitCode",
            args = outfits[i].id
        }
    end
...
RegisterNetEvent("viscosity_appearance:client:OutfitManagementMenu", function(args)
    local outfits = lib.callback.await("viscosity_appearance:server:getManagementOutfits", false, args.type, Framework.GetGender())
    local managementMenuID = "viscosity_appearance_outfit_management_menu"
    local changeManagementOutfitMenuID = "viscosity_appearance_change_management_outfit_menu"
    local deleteManagementOutfitMenuID = "viscosity_appearance_delete_management_outfit_menu"

    RegisterChangeOutfitMenu(changeManagementOutfitMenuID, managementMenuID, outfits, args.type)
    RegisterDeleteOutfitMenu(deleteManagementOutfitMenuID, managementMenuID, outfits, "viscosity_appearance:client:DeleteManagementOutfit")
    local managementMenu = {
        ...
        options = {
            ...
            },
            {
                ...
                event = "viscosity_appearance:client:SaveManagementOutfit",
                args = args.type
            },
            {
                title = _L("outfits.delete.title"),
...
RegisterNetEvent("viscosity_appearance:client:SaveManagementOutfit", function(mType)
    local outfitData = {
        Type = mType,
    ...
    TriggerServerEvent("viscosity_appearance:server:saveManagementOutfit", outfitData)
...
local function RegisterWorkOutfitsListMenu(id, parent, menuData)
    ...
    local event = "viscosity_appearance:client:loadJobOutfit"
    if Config.BossManagedOutfits then
        event = "viscosity_appearance:client:changeOutfit"
    end
    if menuData then
...
function OpenMenu(isPedMenu, menuType, menuData)
    local mainMenuID = "viscosity_appearance_main_menu"
    local mainMenu = {
        id = mainMenuID
    }
    local menuItems = {}

    local outfits = lib.callback.await("viscosity_appearance:server:getOutfits", false)
    local changeOutfitMenuID = "viscosity_appearance_change_outfit_menu"
    local updateOutfitMenuID = "viscosity_appearance_update_outfit_menu"
    local deleteOutfitMenuID = "viscosity_appearance_delete_outfit_menu"
    local generateOutfitCodeMenuID = "viscosity_appearance_generate_outfit_code_menu"
    ...
    RegisterDeleteOutfitMenu(deleteOutfitMenuID, mainMenuID, outfits, "viscosity_appearance:client:deleteOutfit")
    ...
    local outfitMenuItems = {
        ...
        },
        {
            ...
            event = "viscosity_appearance:client:saveOutfit"
        },
        {
            title = _L("outfits.generate.title"),
        ...
        },
        {
            ...
            event = "viscosity_appearance:client:importOutfitCode"
        }
    }
    if menuType == "default" then
        ...
        menuItems[#menuItems + 1] = {
            ...
            event = "viscosity_appearance:client:openClothingShop",
            args = isPedMenu
        }
    ...
    elseif menuType == "job-outfit" then
        ...
        menuItems[#menuItems + 1] = {
            ...
            event = "viscosity_appearance:client:reloadSkin",
            args = true
        }

        local workOutfitsMenuID = "viscosity_appearance_work_outfits_menu"
...
RegisterNetEvent("viscosity_appearance:client:openClothingShopMenu", function(isPedMenu)
...
RegisterNetEvent("viscosity_appearance:client:OpenBarberShop", OpenBarberShop)

RegisterNetEvent("viscosity_appearance:client:OpenTattooShop", OpenTattooShop)

RegisterNetEvent("viscosity_appearance:client:OpenSurgeonShop", OpenSurgeonShop)

RegisterNetEvent("viscosity_appearance:client:changeOutfit", function(data)
    local pedModel = client.getPedModel(cache.ped)
    local appearanceDB
    ...
    if appearanceDB then
        ...
        if data.disableSave then
            TriggerServerEvent("viscosity_appearance:server:syncUniform", {
                type = data.type,
                name = data.name
            }) -- Is a uniform
        else
...
RegisterNetEvent("viscosity_appearance:client:DeleteManagementOutfit", function(id)
    TriggerServerEvent("viscosity_appearance:server:deleteManagementOutfit", id)
    lib.notify({
...
RegisterNetEvent("viscosity_appearance:client:deleteOutfit", function(id)
    TriggerServerEvent("viscosity_appearance:server:deleteOutfit", id)
    lib.notify({
...
RegisterNetEvent("viscosity_appearance:client:openJobOutfitsMenu", function(outfitsToShow)
...
RegisterNetEvent("viscosity_appearance:client:reloadSkin", function(bypassChecks)
...
RegisterNetEvent("viscosity_appearance:client:ClearStuckProps", function()
