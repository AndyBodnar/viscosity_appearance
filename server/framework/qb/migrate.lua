...
local function MigrateQBClothing(source)
    ...
    for i=1, #allPlayerSkins, 1 do
        if not tonumber(allPlayerSkins[i].model) then
            ...
            TriggerClientEvent("viscosity_appearance:client:migration:load-qb-clothing-skin", source, allPlayerSkins[i])
...
RegisterNetEvent("viscosity_appearance:server:migrate-qb-clothing-skin", function(citizenid, appearance)
    local src = source
    ...
    continue = true
    lib.notify(src, {
        id = "viscosity_appearance_skin_migrated",
        title = _L("migrate.success.title"),
