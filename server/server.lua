...
lib.callback.register("viscosity_appearance:server:generateOutfitCode", function(_, outfitID)
...
lib.callback.register("viscosity_appearance:server:importOutfitCode", function(source, outfitName, outfitCode)
...
lib.callback.register("viscosity_appearance:server:getAppearance", function(source, model)
    local citizenID = Framework.GetPlayerID(source)
    return Framework.GetAppearance(citizenID, model)
end)

lib.callback.register("viscosity_appearance:server:hasMoney", function(source, shopType)
...
lib.callback.register("viscosity_appearance:server:payForTattoo", function(source, tattoo)
    local src = source
...
lib.callback.register("viscosity_appearance:server:getOutfits", function(source)
    local citizenID = Framework.GetPlayerID(source)
...
lib.callback.register("viscosity_appearance:server:getManagementOutfits", function(source, mType, gender)
...
lib.callback.register("viscosity_appearance:server:getUniform", function(source)
    return uniformCache[Framework.GetPlayerID(source)]
end)

RegisterServerEvent("viscosity_appearance:server:saveAppearance", function(appearance)
    local src = source
...
RegisterServerEvent("viscosity_appearance:server:chargeCustomer", function(shopType)
    local src = source
    local money = getMoneyForShop(shopType)
...
RegisterNetEvent("viscosity_appearance:server:saveOutfit", function(name, model, components, props)
    local src = source
    local citizenID = Framework.GetPlayerID(src)
    if outfitCache[citizenID] == nil then
        getOutfitsForPlayer(citizenID)
...
RegisterNetEvent("viscosity_appearance:server:updateOutfit", function(id, model, components, props)
    local src = source
    local citizenID = Framework.GetPlayerID(src)
    if outfitCache[citizenID] == nil then
        getOutfitsForPlayer(citizenID)
...
RegisterNetEvent("viscosity_appearance:server:saveManagementOutfit", function(outfitData)
    local src = source
...
RegisterNetEvent("viscosity_appearance:server:deleteManagementOutfit", function(id)
    Database.ManagementOutfits.DeleteByID(id)
end)

RegisterNetEvent("viscosity_appearance:server:syncUniform", function(uniform)
    local src = source
    uniformCache[Framework.GetPlayerID(src)] = uniform
end)

RegisterNetEvent("viscosity_appearance:server:deleteOutfit", function(id)
    local src = source
    local citizenID = Framework.GetPlayerID(src)
...
RegisterNetEvent("viscosity_appearance:server:resetOutfitCache", function()
    local src = source
    local citizenID = Framework.GetPlayerID(src)
    if citizenID then
...
RegisterNetEvent("viscosity_appearance:server:ChangeRoutingBucket", function()
    local src = source
    SetPlayerRoutingBucket(src, src)
end)

RegisterNetEvent("viscosity_appearance:server:ResetRoutingBucket", function()
    local src = source
...
if Config.EnablePedMenu then
    lib.addCommand("pedmenu", {
        ...
        TriggerClientEvent("viscosity_appearance:client:openClothingShopMenu", target, true)
    end)
end

if Config.EnableJobOutfitsCommand then
    lib.addCommand("joboutfits", { help = _L("commands.joboutfits.title"), }, function(source)
        TriggerClientEvent("viscosity_appearance:client:outfitsCommand", source, true)
    end)

    lib.addCommand("gangoutfits", { help = _L("commands.gangoutfits.title"), }, function(source)
        TriggerClientEvent("viscosity_appearance:client:outfitsCommand", source)
    end)
end

lib.addCommand("reloadskin", { help = _L("commands.reloadskin.title") }, function(source)
    TriggerClientEvent("viscosity_appearance:client:reloadSkin", source)
end)

lib.addCommand("clearstuckprops", { help = _L("commands.clearstuckprops.title") }, function(source)
    TriggerClientEvent("viscosity_appearance:client:ClearStuckProps", source)
end)
